        ...
        for event, elem in tree:
            if event == "end" and elem.tag == "Sentence":
                ...
                if story.attrib['sentence'] == 'True':
                    ...
                    text = text.replace('\n', '').replace('  ', '')
                    sent_dict = {'sentence': text, 'parsed': parsed_content}
                    ...
                    for i, sent in enumerate(split_sents[:7]):
                        sent_dict[i] = {'sentence': sent, 'parsed':
                                        parsed_content}
...
def read_single_input(entry):
    holding = {}

    entry_id = str(entry['_id'])
    doc_id = str(entry['doc_id'])
    meta_content = {'date': Utilities._format_datestr(doc_id[8:16]),
                     'source': doc_id.split('_')[0]
                   }

    if 'corref' in entry:
        corefs = entry['corref']
        meta_content.update({'corref': corefs})

    if entry['sentences']:
        dict = entry['sentences'][0]
        if 'parse_sentence' in dict:
            parsetrees = dict['parse_sentence']
        else:
            parsetrees = ''

        if 'sentence' in dict:
            split_sents = _sentence_segmenter(dict['sentence'])
            # TODO Make the number of sents a setting
            sent_dict = {}
            for i, sent in enumerate(split_sents[:7]):
                if parsetrees:
                    try:
                        tree = Utilities._format_parsed_str(parsetrees)
                    except IndexError:
                        tree = ''
                    sent_dict[i] = {'sentence': sent, 'parsed': tree}
                else:
                    sent_dict[i] = {'sentence': sent}
