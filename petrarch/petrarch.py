...
def do_coding(event_dict, out_file):
    ...
    for key, val in event_dict.items():
        ...
        for sent in val['sents']:
            ...
            if 'parsed' in event_dict[key]['sents'][sent]:
                ...
                SentenceText = event_dict[key]['sents'][sent]['sentence']
    ...
    NTotalDoc = NStory
    NIgnoreDoc = NEmptySents + NDiscardStory
    NConsiderDoc = NTotalDoc - NIgnoreDoc
    ...
    print(
        ...
        NNoEvents)
    print("Parse Error:",NParseError)
    print("Empty Sentences:", NEmptySents)
    print("Total Sentences:", NTotSents)

    print("Total Documents", NTotalDoc)
    print("Ignore Documents", NIgnoreDoc)
    print("Considered Documents", NConsiderDoc)

    print("Skipped Documents", NSkippedDoc)
    print("No Event Documents", NNoEventDoc)
    print("Event Documents", NEventDoc)
...
def main():
    ...
    if cli_args.command_name == 'parse' or cli_args.command_name == 'batch':
        ...
        if cli_args.config:
            ...
            PETRreader.parse_Config(Utilities._get_data('/opt/petrarch-master/petrarch/data/config',
                                                        'PETR_config.ini'))
...
def read_dictionaries(validation=False):
    ...
    if validation:
        verb_path = Utilities._get_data(
            pathDir + '/petrarch-master/petrarch/data/dictionaries/',
    ...
    for actdict in PETRglobals.ActorFileList:
        actor_path = Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/dictionaries/', actdict)
        PETRreader.read_actor_dictionary(actor_path)

    print('Agent dictionary:', PETRglobals.AgentFileName)
    agent_path = Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/dictionaries/',
                                     PETRglobals.AgentFileName)
    PETRreader.read_agent_dictionary(agent_path)

    print('Discard dictionary:', PETRglobals.DiscardFileName)
    discard_path = Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/dictionaries/',
    ...
    if PETRglobals.IssueFileName != "":
        ...
        issue_path = Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/dictionaries/',
...
def run_from_spark(data):
    new_results = []
    for i, story in enumerate(list(data)):
        new_results.append(story)
    spark_encode_events(data)

def spark_encode_events(x):
    data = json.loads(x)
    events = PETRreader.read_single_input(data)
    updated_events = do_coding(events, 'TEMP')

    #write_output = True
    #out_file="encoded_output.txt"
...
def init_dictionary():
    ...
    logger = logging.getLogger('petr_log')
    logger.info('Using default config file.')
    logger.info('Config path: {}'.format(Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/config','PETR_config.ini')))
    PETRreader.parse_Config(Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/config','PETR_config.ini'))
    read_dictionaries()
...
def run_pipeline(data, out_file=None, config=None, write_output=True,
    ...
    if config:
        ...
        logger.info('Using default config file.')
        logger.info('Config path: {}'.format(Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/config','PETR_config.ini')))
        PETRreader.parse_Config(Utilities._get_data(pathDir + '/petrarch-master/petrarch/data/config',
                                                    'PETR_config.ini'))
